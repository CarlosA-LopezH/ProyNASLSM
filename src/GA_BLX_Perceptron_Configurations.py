# General imports
from numpy.random import random as npRandom, randint as npRandInt, set_state as npSetstate, get_state as npGetstate, choice as npChoice
...
class Enc_Configuration:
    ...
    def __init__(self, n_neurons: int, channels: int, positions: list, dim: float = 6.) -> None:
        # General parameters
        ...
        self.positions: list[list] = positions
        # Random encoding
    ...
    def polarity_indexing(self) -> None:
        ...
        while len(exc_indexes) <= 1 or len(inh_indexes) <= 1:
            ...
            if len(exc_indexes) <= 1:
                ...
                # The new neuron needs a position. This is different that lib.encoding since position is handle outside the function.
                # Flag to force valid (no repeated) position.
                invalid: bool = True
                # Initialize coordinates
                x, y = 0., 0.
                while invalid:
                    x = round(uniform(0, 6), 2)  # X position.
                    y = round(uniform(0, 6), 2)  # Y position.
...
def initEncoding(encoding: Enc_Configuration, dimensions: tuple | int, channels: int, positions: list) -> Enc_Configuration:
    ...
    return encoding(n_neurons, channels, positions)

def xover_blx(parent1: Enc_Configuration, parent2: Enc_Configuration, alpha: float = 0.5) -> tuple[Enc_Configuration, Enc_Configuration]:
    """
...
def mutation_simple(individual: Enc_Configuration) -> Enc_Configuration:
    """
    Mutation. One type of mutation operation:
    ...
    mut_size: int = int((individual.nT / 2) * npRandom())
    # Indexes of the neurons to be mutated.
    mut_neurons = npChoice(range(individual.nT), mut_size, replace=False)
